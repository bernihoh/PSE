...
%Graphen von Hand bearbeiten:
%Vorbedingung: Ein fertig generierter Graph muss existieren.
%Nachbedingung: Falls Änderungen am Graphen vorgenommen wurden, wird die Änderung
%	       als Version direkt neben dem Ursprungsgraphen in der Previw gepeichert
%               und die Zusammengehörigkeit mit einer Schraffierung gekenzeichnet.
%1.)  User öffnet einen Graphen in der Deatilansicht.
%     User drückt auf "Bearbeiten.
%2.)  Graph wird im Editor geöffnet.
%3.)  User kann mithilfe von Tools den Graphen bearbeiten.
%4.)  Sobald User auf "OK" drückt, wird der modifizierte Graph in die Detailansicht geladen
%     und auch als Version in der Preview, wie in der Nachbedingung beschrieben, gespeichert.

	
%Graphen Speichern:
%Vorbedingung: Es müssen >=1 Graphen in Preview sein.
%Nachbedingung: Graphen werden korrekt in einer Datei kodiert und auf der Festplatte gespeichert.

%1.)  User drückt in der GUI unten rechts auf -->"Graphen" --> "Graphen Speichern".
%2.)  User wählt die zu speichernden Graphen aus und Bestätigt. //(Es muss noch im Entwurf spezifiziert werden, ich würde sagen sowie die Bilderauswahl in WIN10, indem man mit der Maus ein Kasten zieht.)
%3.)  Die Graphen werden in einer Datei kodiert und auf der Festplatte gespeichert.
%--> Hier gehört kein Workflowdiagramm dazu.
...
%Vorbedingung: Es müssen >=1 Graphen koloriert sein.
%Nachbedingung: Es werden verscheidene Statistiken angezeigt, die Informationen über die Heuristiken und Graphen beinhalten.

%1.)  Während der Kolorierung und Generierung sammelt das Programm Daten über die Graphen und deren Färbung,
%     sowie die Effizienz und Erfolg der Heuristiken.
%2.)  Nach der kolorierung der Graphen drückt der User auf "Statistiken".
%3.)  Das Programm liest die gesammelten Daten aus und stellt sie Graphisch in verscheidenen Statistiken dar.
%--> Hier kommt kein Workflowdiagramm rein.
	

%Grapheneinstellunge Speichern:
%Vorbedingung: Es müssen Grapheingenschaften eingestellt sein.
%Nachbedingung: Grapheneinstellungen werden korrekt in einer Datei kodiert und auf der Festplatte gespeichert.

%1.)  User drückt in der GUI unten rechts auf -->"Graphen" --> "Grapheneisntellungen Speichern".
...
%Vorbedingung: Es müssen Heuristiken eingestellt sein.
%Nachbedingung: Heuristiken werden korrekt in einer Datei kodiert und auf der Festplatte gespeichert.

%1.)  User drückt in der GUI unten rechts auf -->"Hilfe & Sonstiges" --> "Heuristikeinstellungen Speichern".
...
%Nachbedingung (Erfolg): Heuristikeinstellungen werden aus Datei gelesen und in die Heuristikauswahl übernommen.
%Nachbedingung (Misserfolg): Aussagekräftige Fehlermeldung erscheint.

%1.)  User wählt -->"Hilfe & Sonstiges" --> "Heuristeinstellungen importieren" und gibt Pfad zu Datei an.
...
%3.1) Bei Erfolg werden die Heuristiken in die globale/lokale Heuristikauswahl übernommen.
