...
.controller('Hello', function($scope, $http) {	
    $http.get('http://node159550-env-7559099.jelastic.dogado.eu:11022/graph').
	 //$http.get('http://localhost:8090/greeting').
        then(function(response) {   
        	var response = $http.get('http://node159550-env-7559099.jelastic.dogado.eu:11022/getGraphTypeList').
				then(function(response) {
		...
		$scope.test = function() {  
			...
			$scope.s.refresh();								
			//n.color = '#000';
			var response = $http.get('http://node159550-env-7559099.jelastic.dogado.eu:11022/runHeuristic',{params: {'name': $scope.selectedHeuristic}}).								
			then(function(response) {
				$scope.re = response;
				var i;
				$scope.colors = [];				
		...
		$scope.genGraph = function() {			
		   ...
		   // cpParams.set("maxDegree", maxDegree);  
			...
			var response = $http.get('http://node159550-env-7559099.jelastic.dogado.eu:11022/genGraph',{params: {'nOfVertices': nOfVertices,'maxDegree':maxDegree,
			...
			var i;			
			$scope.s.refresh();			
			var response = $http.get('http://node159550-env-7559099.jelastic.dogado.eu:11022/getHeuristicList').
				then(function(response) {
		...
		$scope.bfs = function() {
			...
			$scope.dis_heuristic = false;
			var response = $http.get('http://node159550-env-7559099.jelastic.dogado.eu:11022/bfs').
				then(function(response) {
