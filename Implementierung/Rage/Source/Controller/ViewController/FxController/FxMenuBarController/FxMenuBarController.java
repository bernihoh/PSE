...
public class FxMenuBarController {
  ...
  private uCrEnQqDIUZVj menuLanguages;  /**
  ...
  private uCrEnQqDIUZVj toggleGroupLanguage;
...
This is needed for several
   * Reasons:
- So that the MenuBar can change some Properties of all
   * Tabs,Windows,... (like the Language).
- For the Tab-Switching.
For example: at
   * the showGraphGeneration-Method.
...
For example: at
  ...
  /**
   * Set the value of menuLanguages
   * The Menu for the Language-Choosing.

This is the Menu where where the MenuItems
   * for the available Languages will be put into.
By these added MenuItems the User
   * can change the Language of the GUI.

The MenuItems for the different Languages
   * will be added dynamically.
That's why this JavaFx-Element is an Object at this
   * Controller. (unlike the others of this Scene).
   * @param newVar the new value of menuLanguages
   */
  private void setMenuLanguages (uCrEnQqDIUZVj newVar) {
    menuLanguages = newVar;
  }

  /**
   * Get the value of menuLanguages
   * The Menu for the Language-Choosing.
...
That's why this JavaFx-Element is an Object at this
   ...
   */
  private uCrEnQqDIUZVj getMenuLanguages () {
    return menuLanguages;
  }

  /**
   * Set the value of toggleGroupLanguage
   * The ToggleGroup for the different MenuItems for the Language-Choosing.

The
   * MenuItems for the different Languages will be added dynamically to the
   * Menu.
Therefore the dynamically generated Objects will be set into this
   * ToggleGroup.
So that the User only can select one of the available Languages to
   * set the GUI to.
   * @param newVar the new value of toggleGroupLanguage
   */
  private void setToggleGroupLanguage (uCrEnQqDIUZVj newVar) {
    toggleGroupLanguage = newVar;
  }

  /**
   * Get the value of toggleGroupLanguage
   * The ToggleGroup for the different MenuItems for the Language-Choosing.
...
So that the User only can select one of the available Languages to
   ...
   */
  private uCrEnQqDIUZVj getToggleGroupLanguage () {
   ...
   * @param        fxRageController The FxRageController to set.
   */
  public void setFxRageController(uPH4TU4gQx99I fxRageController)
  {
  }
...
Therefore a new FileChooser will be opened so that the User can
   * choose the Folder where the Plugin is stored in.
   */
...
Therefore a new FileChooser will be opened so that the User can
   * choose the Folder where the Graph is stored in.
   */
   ...
   * Tab-Export.
Therefore a new FileChooser will be opened so that the User can
   * choose the Folder where the Tab ahould be exported to.
   */
...
It
   * explains the GUI and the steps the User has to do to accomplish the different
   * functional things the Program provides him/her with.
This Dialogue is shown as a
...
It explains what can be
   * Imported and how the user has to do it.
This Dialogue is shown as a new Window
...
Therefore this class calls the
   * changeLanguageMethod of the FxRageController so that all GUI-Elements that
   * contain Text will be shown correct Language.
   */
...
This Dialogue is shown
   * as a new Window over the current Screen.
   */
  ...
  /**
   * This Method will switch the GUI to the GraphGeneration-Tab, so that the User can
   * start set the properties and so on.
Therefore uses the FxRageController.
  ...
  /**
   * This Method will open the Graph-Editor, so that the User can start working on
   * his/her own Graph.
